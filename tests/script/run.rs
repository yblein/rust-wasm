    ...
    for cmd in parser {
        match cmd {
            Cmd::ModuleSource(src) => {
                ...
                let export_names: Vec<String> = module_exports(&m)
            ...
            Cmd::Register { name, mod_ref } => {
                let mod_name = if mod_ref.is_some() {
                    &mod_ref
                } else {
                    &registry.last_key
                };
            ...
            match val {
                values::Value::F32(f)
                    if f.to_bits() == f32::NAN.to_bits()
                        || f.to_bits() == (-f32::NAN).to_bits() => {}
                values::Value::F64(f)
                    if f.to_bits() == f64::NAN.to_bits()
                        || f.to_bits() == (-f64::NAN).to_bits() => {}
                _ => {
                    panic!(
        ...
        Malformed(mod_src, _) => {
            match mod_src {
                ModuleSource::Binary(_, bytes) => {
                    assert_eq!(
                        decode_module(Cursor::new(bytes)).unwrap_err(),
                        Error::DecodeModuleFailed
                    );
                }
...
fn init_spectest(store: &mut Store, registry: &mut Registry) {
    ...
    symbols.insert(
        ...
        print(store, vec![types::I32, types::F32]),
    );
    ...
    symbols.insert(
        ...
        print(store, vec![types::F64, types::F64]),
    );
    symbols.insert(
        "global_i32".to_owned(),
        global(store, values::Value::I32(666)),
    );
    symbols.insert(
        "global_f32".to_owned(),
        global(store, values::Value::F32(666.0)),
    );
    symbols.insert(
        "global_f64".to_owned(),
        global(store, values::Value::F64(666.0)),
    );

    registry
        .mod_exports
        .insert(Some(String::from("spectest")), symbols);
}
    ...
    module_imports(m)
        .map(|(mod_name, import_name, _)| {
            match registry.mod_exports.get(&Some(mod_name.to_owned())) {
                ...
                None => Err(UnknownImport::UnknownModule {
                    mod_name: mod_name.to_owned(),
                }),
            }
        })
