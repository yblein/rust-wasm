...
impl Interpreter {
    ...
    fn call_host(
        ...
        self.return_buffer
            .resize(f_inst.type_.result.len(), Value::false_());
    ...
    fn load(
        ...
        let res = match (size_in_bits, signed, memop.type_) {
            ...
            (32, false, Tv::Int(Int::I32)) => Value::I32(u32::from_bits(bits)),
            ...
            (64, false, Tv::Int(Int::I64)) => Value::I64(u64::from_bits(bits)),
    ...
    fn store(
        ...
        match (size_in_bits, memop.type_, c) {
            ...
            (32, Tv::Int(Int::I32), Value::I32(c)) => c.to_bits(bits),
            ...
            (64, Tv::Int(Int::I64), Value::I64(c)) => c.to_bits(bits),

            (32, Tv::Float(Float::F32), Value::F32(c)) => c.to_bits().to_bits(bits),
            (64, Tv::Float(Float::F64), Value::F64(c)) => c.to_bits().to_bits(bits),
            _ => unreachable!(),
        };
        Ok(Continue)
    }
