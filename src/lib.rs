...
pub fn invoke_func(
    ...
    let mut int = interpreter::Interpreter::new(contract_data, msg_data);
...
pub fn instantiate_module(
    ...
    for (&extern_val, import) in extern_vals.iter().zip(module.imports.iter()) {
        ...
        if !ext_type.matches(&import.type_(&module)) {
            println!("ext: {:?} import: {:?}", ext_type, import.type_(&module));
...
fn allocate_and_init_module(
    ...
    if let Some(idx) = module.start {
        ...
        invoke_func(store, func_addr, Vec::new(), None, None)?;
    }
